...
async def add_product_to_cart_by_id(product_id: int, user_id: int) -> str:
    product = await get_product(product_id=product_id)
    await Carts.add_cart_(product_id_=product["id"], user_id_=user_id)
    return "Товар был успешно добавлен в корзинку!"


async def add_order_from_cart(user_id: int) -> str:
    orders = []
    ...
    return "Заказ был успешно оформлен!"
