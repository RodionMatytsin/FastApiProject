...
from main.schemas.response import DefaultResponseList
from main.utils.auth import get_users, get_users_token


@app.get("/api/users", response_model=DefaultResponseList, tags=["user"])
async def read_users():
    data = await get_users()
    return {"result": True, "message": "Успешно, список пользователей был просмотрен!", "data": data}


@app.get("/api/users_token", response_model=DefaultResponseList, tags=["user"])
